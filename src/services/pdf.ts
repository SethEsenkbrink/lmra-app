...
import { PBM_ITEMS } from '../pbm-manager';
...
export const PDFService = {
    async buildDoc(report: LMRAReport): Promise<jsPDF> {
        ...
        let statusColor = isSafe ? SUCCESS_COLOR : DANGER_COLOR;
        let statusBg = isSafe ? SUCCESS_BG : DANGER_BG;
        let statusText = isSafe ? 'VEILIG / GOEDGEKEURD' : 'ONVEILIG / AFGEKEURD';

        if (isHerbeoordeeld) {
            statusColor = { r: 217, g: 119, b: 6 }; // amber-600
            statusBg = { r: 254, g: 243, b: 199 }; // amber-100
            statusText = 'VEILIG NA MAATREGEL (GO)';
        }
        ...
        const pbmList =
            report.pbm && report.pbm.length > 0
                ? report.pbm
                      .map((id) => {
                          const found = PBM_ITEMS.find((p) => p.id === id);
        ...
        yPos = this.tableEndY(doc, yPos) + 10;

        if (isHerbeoordeeld && report.herbeoordeling) {
            const hb = report.herbeoordeling;
            const hbTime = new Date(hb.tijdstip).toLocaleTimeString('nl-NL').slice(0, 5);
            autoTable(doc, {
                startY: yPos,
                theme: 'grid',
                head: [['HERBEOORDELING TER PLAATSE (STOP & GO PROTOCOL)']],
                body: [
                    [
                        toPdfText(
                            `Uitgevoerde maatregel: ${hb.toelichting}\n` +
                                `Herbeoordeeld door: ${hb.verklaard_door} (om ${hbTime})\n` +
                                `Eigen verklaring monteur: De maatregel is ter plaatse uitgevoerd en de werksituatie is veilig bevonden.\n` +
                                `Disclaimer: LMRA Pro is een hulpmiddel. Verantwoordelijkheid voor veilig werken berust bij monteur en werkgever.`
                        ),
                    ],
                ],
                styles: {
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            doc.line(MARGIN, 278, 200, 278);
            doc.setFontSize(7.5);
            doc.setTextColor(140);
            doc.setFont('helvetica', 'normal');
            doc.text(toPdfText(`Rapport ID: ${report.report_id}`), MARGIN, 282);
            doc.text(`LMRA Pro v${APP_VERSION} ${APP_RELEASE_NAME} - Offline PWA`, 105, 282, { align: 'center' });
            doc.text(`Pagina ${i} van ${pageCount}`, 200, 282, { align: 'right' });
            doc.setFontSize(6.5);
            doc.setTextColor(160);
            doc.text(
                toPdfText('LMRA Pro is een hulpmiddel ter ondersteuning. Verantwoordelijkheid voor veilig werken berust bij monteur en werkgever.'),
                105,
                286,
                { align: 'center' }
            );
    ...
    async generate(report: LMRAReport | null): Promise<void> {
        ...
        try {
            const doc = await this.buildDoc(report);
            const dateCreated = new Date(report.created_at);
            ...
            Diagnostics.log('info', 'pdf', `PDF gemaakt in ${ms}ms, ${doc.getNumberOfPages()} pagina(s), ${sizeKb} kB`);
    ...
    async share(report: LMRAReport | null): Promise<void> {
        if (!report) return;

        const started = performance.now();
        UI.showToast('Rapport voorbereiden om te delen...');
        ...
        try {
            ...
            const sizeKb = Math.round(pdfBlob.size / 1024);
            const ms = Math.round(performance.now() - started);
            Diagnostics.log('info', 'pdf', `PDF klaargemaakt om te delen in ${ms}ms (${sizeKb} kB)`);

            const isHerbeoordeeld = report.herbeoordeling?.is_herbeoordeeld === true;
            const statusText = isHerbeoordeeld
                ? 'VEILIG NA MAATREGEL (GO)'
                : report.is_veilig
                ? 'VEILIG / GOEDGEKEURD'
                : 'ONVEILIG / STOP';

            const shareTitle = `LMRA Rapport: ${report.locatie || 'Werkplek'} (${report.werkorder || 'Geen order'})`;
            const shareText = `LMRA Rapportage\nMonteur: ${report.monteur_naam}\nBedrijf: ${report.bedrijf_naam || '-'}\nLocatie: ${report.locatie}\nWerkorder: ${report.werkorder || '-'}\nStatus: ${statusText}\nTaak: ${report.template_label || 'Algemeen'}`;
            ...
            if (navigator.share && navigator.canShare) {
                ...
                if (navigator.canShare({ files: [file] })) {
                    try {
                        ...
                        Diagnostics.log('warn', 'pdf', `Delen bestand mislukt (${name}), fallback naar WhatsApp link`);
                    }
                }
            }
            ...
            const waText = encodeURIComponent(
                `${shareText}\n\n(Bekijk het volledige PDF-rapport ter plaatse op het toestel)`
            );
            const waUrl = `https://wa.me/?text=${waText}`;
            window.open(waUrl, '_blank');
            UI.showToast('Bericht geopend via WhatsApp');
        } catch (err) {
            const msg = err instanceof Error ? `${err.name}: ${err.message}` : String(err);
            Diagnostics.log('error', 'pdf', `Delen mislukt: ${msg}`);
            UI.showToast('❌ Delen mislukt.');
        }
    },
